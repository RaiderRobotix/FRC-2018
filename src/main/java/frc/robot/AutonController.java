...
public final class AutonController {
	...
	/**
	 * Drives straight to the switch and drops the cube if the switch is the proper color.
	 * 
	 * This works for both the right and left sides.
	 * 
	 * If the switch is not the right color, back up and lower the mast to prevent accidentally dropping the cube during the transition from auto -> teleop
	 * 
	 * @param dropCube
	 */
	...
	public void leftSwitchBack() {
		if (m_step == 3) {
			 ...
			 * else if (m_step == 21) { if (m_drives.driveStraightFast(-30.0, 0.75)) { this.nextStep(); } } else if (m_step == 22) { if (m_drives.turnToAngle(-43.0, 0.5)) { this.nextStep(); } } else if (m_step ==
			 * 23) { if (m_drives.driveStraightFast(30.0, 0.75)) { m_grabber.grab(); this.nextStep(); } } else if (m_step == 24) { if (m_timer.get() > 0.25) { if
			 * (m_elevator.goToHeight(Constants.ELEVATOR_SWITCH_PRESET)) { this.nextStep(); } } } else if (m_step == 25) { if (m_drives.driveStraightFast(10.0, 0.5)) { m_grabber.release(); this.nextStep(); } }
			 * else if (m_step == 26) { if (m_timer.get() > 0.25) { if (m_drives.driveStraightFast(-24.0, 0.5)) { this.nextStep(); } } } else if (m_step == 27) { if
			 * (m_elevator.goToHeight(Constants.ELEVATOR_SWITCH_PRESET)) { this.nextStep(); } }
			 */ else {
			this.nextStep();
		}
	}
	...
	public void practiceBox(double sideLength) {
		...
		case 0:
			...
			break;
		case 1:
			goNext = m_drives.driveStraight(sideLength, speed);
			break;
		case 2:
			goNext = m_drives.turnToAngle(angle, turnSpeed);
			break;
		case 3:
			goNext = m_drives.driveStraight(sideLength, speed);
			break;
